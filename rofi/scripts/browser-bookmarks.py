...
cache_dir = Path(environ.get('XDG_CACHE_HOME',
                 Path.home() / '.cache')) / 'rofi-bookmarks'
firefox_dir = Path.home() / '.librewolf/'
    ...
    with temp_sqlite(profile_loc / 'favicons.sqlite') as favicons:
        for index, parent, type, title, url in conn_res:
            if type == 1:  # type one means bookmark
                ...
                path_arr = reversed(list(parent_generator(index)))
                ...
                if all(name == next(path_arr) for name in search_path):
                    ...
                    if icon:
                        print(
                            f"{path}\x00info\x1f{url}\x1ficon\x1f{cache_icon(icon)}")
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('path',              default="",
                        nargs='?',      help="restrict list to a bookmark folder")
    parser.add_argument('-s', '--separator', default=" / ",
                        metavar='sep',  help="seperator for paths")
    parser.add_argument('-p', '--profile',
                        metavar='prof', help="firefox profile to use")
    ...
    if environ.get('ROFI_RETV') == '1':
        ...
        subprocess.Popen(["librewolf", environ['ROFI_INFO']] + prof,
                         close_fds=True, start_new_session=True, stdout=subprocess.DEVNULL)
    else:
        search_path = [i for i in args.path.split('/') if i != '']
        profile_path = default_profile_path(
        ) if args.profile is None else path_from_name(args.profile)

        print("\x00prompt\x1f ")  # change prompt
        write_rofi_input(profile_path, search_path=search_path,
                         sep=args.separator)
